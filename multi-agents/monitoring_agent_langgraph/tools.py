...
retrieve_runbook() now reads the REAL runbook markdown files off local disk
instead of a hardcoded dict. This is still a stand-in, not the real
pipeline: the actual production path is upload these same .md files to S3,
create a Bedrock Knowledge Base pointed at that bucket, and call
bedrock-agent-runtime.retrieve() against it (Step 2). Reading them from
disk here just lets us test against the real runbook content, with the
real prompts it produces, before that infrastructure exists. The function
signature and return shape stay the same when it's swapped for the real
Bedrock call, so nothing in agent.py needs to change later.
"""
